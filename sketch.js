...
function initGrid() {
  ...
  // for (let circle = 1; circle <= nCircles; circle++) {
      for (let s = 1; s <= nMaxShapes /* / nCircles*/; s++) {
        let c = Math.round(Math.min(Math.random(), Math.random()) * (gridCols - 1));
        let r = Math.round(Math.max(Math.random(), Math.random()) * (gridRows - 1));
...
function keyTyped() {
  if (key === 'p' || key === 'P') {
    ...
    dl.download = "GetAttention-" + getTimeStamp() + ".svg";
